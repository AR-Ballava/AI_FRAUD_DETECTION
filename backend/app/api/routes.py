...
from app.services.osint import run_osint
...
async def _analyze_payload(request: Request, text: str | None, source_type: str, base64_pdf: str | None = None) -> AnalysisResponse:
    settings = get_settings()
    started = time.perf_counter()

    text_entities = extract_entities(text or "")
    model_payload = {"source_type": source_type}
    if base64_pdf:
        model_payload["base64_pdf"] = base64_pdf
    else:
        model_payload["text"] = sanitize_text(text or "", settings.max_text_chars)
    ...
    if osint_triggered:
        osint = await run_osint(entities, initial_fraud_score=fraud_score, timeout=settings.scrape_timeout)
...
async def upload(
    ...
    try:
        file_kind = validate_file_upload(file.filename or "upload", file.content_type, data, settings.max_file_size)
        if file_kind == "pdf":
...
async def osint(payload: OsintRequest, request: Request) -> dict:
    ...
    if payload.text:
        try:
            clean_text = sanitize_text(payload.text, settings.max_text_chars)
        except ValueError as exc:
            raise HTTPException(status_code=413, detail=str(exc)) from exc
        text_entities = extract_entities(clean_text)

    entities = merge_entities(payload.entities, text_entities)
    if not any(entities.get(key) for key in ("emails", "phones", "domains", "urls", "companies", "recruiters", "positions")):
        raise HTTPException(status_code=422, detail="Manual OSINT requires at least one company, email, domain, phone, recruiter, or job role")

    fraud_score = payload.fraud_score or settings.fraud_osint_threshold
    osint_result = await run_osint(entities, fraud_score, timeout=settings.scrape_timeout)
